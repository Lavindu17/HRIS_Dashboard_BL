...
export default function EvmDashboard() {
  ...
  const project = useMemo(() => DATA.find((p) => p.id === selectedId), [selectedId]);
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 22, fontWeight: 700 }}>Project S-Curve Dashboard</div>
        </div>
        <div style={{ display: "flex", gap: 6, background: COLORS.panel, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: 4 }}>
          {[
            { id: "dashboard", label: "Dashboard" },
            { id: "banana", label: "Banana Curve" },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setPage(tab.id)}
              style={{
                border: "none", cursor: "pointer", padding: "8px 16px", borderRadius: 7,
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        <div style={{
          width: 260, flexShrink: 0, background: COLORS.panel, border: `1px solid ${COLORS.border}`,
          borderRadius: 12, padding: 10,
        }}>
          <div style={{ fontSize: 11, letterSpacing: "0.06em", color: COLORS.textFaint, textTransform: "uppercase", padding: "6px 8px 10px" }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  borderLeft: active ? `3px solid ${COLORS.ev}` : "3px solid transparent",
                  borderRadius: 8, padding: "10px 10px", marginBottom: 4,
                }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                  <StatusDot status={p.status} />
                  <span style={{ fontSize: 13, fontWeight: 600, color: active ? COLORS.text : COLORS.textMuted }}>{p.name}</span>
                </div>
                <div style={{ fontSize: 11, color: COLORS.textFaint, paddingLeft: 16, lineHeight: 1.4 }}>{p.scenario}</div>
              </button>
            );
          })}
        </div>
